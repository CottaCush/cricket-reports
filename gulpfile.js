...
gulp.task('build', series(cleanStyles, series(series(lessStyles, scssStyles), minifyStyles)));
...
function lessStyles() {
    let sConfig = config.styles,
        files = sConfig.lessFiles,
        source = sConfig.lessDir,
...
lessStyles.description = 'Compiles Less files to CSS; adds source maps if specified';

function scssStyles() {
    let sConfig = config.styles,
        files = sConfig.scssFiles,
        source = sConfig.scssDir,
        dest = sConfig.destinationDir,
        mapsDir = sConfig.mapsDir,
        postcssConfig = sConfig.postcss;
...
scssStyles.description = 'Compiles Sass files to CSS; adds source maps if specified';
...
function watch(done) {
    if (development()) {
        gulp.watch(path.join(config.styles.lessDir, '**/*.less'), lessStyles);
